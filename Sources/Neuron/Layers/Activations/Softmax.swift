...
public final class Softmax: BaseActivationLayer {
  ...
  public override func forward(tensor: Tensor, context: NetworkContext = .init()) -> Tensor {
    let context = TensorContext { inputs, gradient, wrt in
      let wrtInputGradient = Tensor(storage: ContiguousArray(gradient.storage), size: gradient._size)
    ...
    var result = ContiguousArray<Tensor.Scalar>(repeating: 0, count: src.count)
