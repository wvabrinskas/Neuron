...
class LSTMCell {
  ...
  func backward(cache: LSTM.Cache,
    ...
    cellError = cellError * self.device.derivate(tanActivationOfCellActivation, .tanh, inputSize: TensorSize(array: tanActivationOfCellActivation.shape))
    ...
    // gate gate error
    var eg = cellError * ia
    eg = eg * self.device.derivate(ga, .tanh, inputSize: TensorSize(array: ga.shape))
