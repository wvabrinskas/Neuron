...
final class TensorMathTests: XCTestCase {
  ...
  func test_split_dataIntegrity() {
    ...
    XCTAssertTrue(split[0].isValueEqual(to: Tensor<N>([[1,1,1],
                                                    [2,2,2]])))
    XCTAssertTrue(split[1].isValueEqual(to: Tensor<N>([[5,3,5],
                                                    [5,3,5]])))
    // axis 0
    let split1 = tensor.split(into: 1, axis: 0)
    XCTAssertEqual(split1.shape, [2])
    XCTAssertTrue(split1[0].isValueEqual(to: Tensor<N>([[[1,1,1]],
                                                     [[5,3,5]]])))
    XCTAssertTrue(split1[1].isValueEqual(to: Tensor<N>([[[2,2,2]],
                                                     [[5,3,5]]])))
    
    let split2 = tensor.split(into: 1, axis: 1)
    XCTAssertEqual(split2.shape, [3])
    XCTAssertTrue(split2[0].isValueEqual(to: Tensor<N>([[[1.0],[2.0]], [[5.0], [5.0]]])))
    XCTAssertTrue(split2[1].isValueEqual(to: Tensor<N>([[[1.0],[2.0]], [[3.0], [3.0]]])))
    XCTAssertTrue(split2[2].isValueEqual(to: Tensor<N>([[[1.0],[2.0]], [[5.0], [5.0]]])))
  }
  
  func test_split() {
    let tensor = Tensor<N>(NumSwift.onesLike((10, 10, 10)))
  ...
  func test_sum() {
    ...
    XCTAssertTrue(Tensor<N>([35.0]).isValueEqual(to: result))
    
    // axis 0
    let resultZero = tensor.sum(axis: 0)
    let expected = Tensor<N>([[[3.0, 3.0, 3.0]],
                           [[10.0, 6.0, 10.0]]])
    XCTAssertTrue(expected.isValueEqual(to: resultZero))
    
    // axis 1
    let resultOne = tensor.sum(axis: 1)
    let expectedOne = Tensor<N>([[[3.0], [6.0]],
                              [[13.0], [13.0]]])
    XCTAssertTrue(expectedOne.isValueEqual(to: resultOne))
    
    // axis 2
    let resultTwo = tensor.sum(axis: 2)
    let expectedTwo = Tensor<N>([[6.0, 4.0, 6.0],
                              [7.0, 5.0, 7.0]])
    XCTAssertTrue(expectedTwo.isValueEqual(to: resultTwo))
  }
  ...
  func test_subtract() {
    ...
    let expected = Tensor<N>([[[-1.0, -1.0, -1.0]],
                           [[0.0, 0.0, 0.0]]])
    XCTAssertTrue(expected.isValueEqual(to: resultZero))
    ...
    let expectedTwo = Tensor<N>([[-4.0, -2.0, -4.0],
                              [-3.0, -1.0, -3.0]])
    XCTAssertTrue(expectedTwo.isValueEqual(to: resultTwo))
  }
  ...
  func test_multiply() {
    ...
    XCTAssertTrue(Tensor<N>([45000.0]).isValueEqual(to: result))
    
    // axis 0
    let resultZero = tensor.multiply(axis: 0)
    let expected = Tensor<N>([[[2.0, 2.0, 2.0]],
                           [[25.0, 9.0, 25.0]]])
    XCTAssertTrue(expected.isValueEqual(to: resultZero))
    
    // axis 1
    let resultOne = tensor.multiply(axis: 1)
    let expectedOne = Tensor<N>([[[1.0], [8.0]],
                              [[75.0], [75.0]]])
    XCTAssertTrue(expectedOne.isValueEqual(to: resultOne))
    
    // axis 2
    let resultTwo = tensor.multiply(axis: 2)
    let expectedTwo = Tensor<N>([[5.0, 3.0, 5.0],
                              [10.0, 6.0, 10.0]])
    XCTAssertTrue(expectedTwo.isValueEqual(to: resultTwo))
  }
  ...
  func test_norm() {
    ...
    XCTAssertTrue(Tensor<N>([11.532562]).isValueEqual(to: result))
    
    // axis 0
    let resultZero = tensor.norm(axis: 0)
    let expected = Tensor<N>([[[2.236068, 2.236068, 2.236068]],
                           [[7.071068, 4.2426405, 7.071068]]])
    XCTAssertTrue(expected.isValueEqual(to: resultZero))
    
    // axis 1
    let resultOne = tensor.norm(axis: 1)
    let expectedOne = Tensor<N>([[[1.7320508], [3.4641016]],
                              [[7.6811457], [7.6811457]]])
    XCTAssertTrue(expectedOne.isValueEqual(to: resultOne))
    
    // axis 2
    let resultTwo = tensor.norm(axis: 2)
    let expectedTwo = Tensor<N>([[5.0990195, 3.1622777, 5.0990195],
  ...
  func test_sumOfSquares() {
    ...
    XCTAssertTrue(Tensor<N>([133.0]).isValueEqual(to: result))
    
    // axis 0
    let resultZero = tensor.sumOfSquares(axis: 0)
    let expected = Tensor<N>([[[5.0, 5.0, 5.0]],
                           [[50.0, 18.0, 50.0]]])
    XCTAssertTrue(expected.isValueEqual(to: resultZero))
    
    // axis 1
    let resultOne = tensor.sumOfSquares(axis: 1)
    let expectedOne = Tensor<N>([[[3.0], [12.0]],
                              [[59.0], [59.0]]])
    XCTAssertTrue(expectedOne.isValueEqual(to: resultOne))
    
    // axis 2
    let resultTwo = tensor.sumOfSquares(axis: 2)
    let expectedTwo = Tensor<N>([[26.0, 10.0, 26.0],
                              [29.0, 13.0, 29.0]])
    XCTAssertTrue(expectedTwo.isValueEqual(to: resultTwo))
  }
}
