...
- **LSTM BPTT bound (`recurrentErrorClip`)**: `LSTM` no longer `l2Normalize`s its update deltas in `apply()` (that discarded Adam's scaling and never annealed). Instead `LSTM.backward` rescales the hidden/cell errors carried between timesteps to a max L2 norm (`recurrentErrorClip`, default `1.0`, `nil` disables, serialized). See `LSTM.swift` `clipRecurrentError`.
