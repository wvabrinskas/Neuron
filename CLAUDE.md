...
- **LSTM BPTT bound (`recurrentErrorClip`)**: `LSTM` no longer `l2Normalize`s its update deltas in `apply()` (that discarded Adam's scaling and never annealed). Instead `LSTM.backward` rescales the hidden/cell errors carried between timesteps to a max L2 norm (`recurrentErrorClip`, default `nil`/disabled, opt-in via `LSTM.init` or `RNN.RNNLSTMParameters`, serialized). See `LSTM.swift` `clipRecurrentError`.
...
12. **`gradientClip` cannot bound updates in front of Adam**: global-norm clipping scales every gradient by `c` before the moment update, and Adam's `mHat/sqrt(vHat)` cancels `c`. If the clip fires every step it does nothing to the weight trajectory (and pre-fix, the eps floor made it *freeze* the non-exploding parameters). Bound recurrent gradients at the source (`LSTM.recurrentErrorClip`) and use `gradientNormInspector` to find which parameter group is exploding.
